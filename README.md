...
2. 画面下に並んだ集落名カードを、地図上の○の位置へドラッグ&ドロップ（またはカードをタップして選択→地図上の○をタップ）
3. 正しい場所なら固定され、集落名が地図に刻まれます
4. 全部置けたらクリア

スマホのタッチ操作・PCのマウス操作・キーボードのどれでも遊べます。

### キーボードで遊ぶ

マウスに触れずに最後まで遊べます。キーを押すと画面上部に操作の案内が出ます。

| キー | 動き |
| --- | --- |
| `Tab` | ステージタブ → 地図の○ → 集落カード の順に移動 |
| `←` `→` `↑` `↓` | カードや地図の○を選ぶ（○は押した方向にある○へ移ります） |
| `Enter` / `Space` | カードの上なら「選択」して地図へ、○の上なら「配置」 |
| `Esc` | 選択の取り消し／クリア画面を閉じる |

正しい○に置くと、自動でトレイへ戻って次のカードを選べます。
配置済みの○で `Enter` を押すと、その集落の豆知識をもう一度読めます。

スクリーンリーダーでも遊べます（未配置の○は答えが漏れないよう「配置場所 N」とだけ読み上げ、
選択・正解・不正解・残り数はその都度読み上げます）。
...
Claude Design側の `dist/` から `index.html` を取り込む際は、`<head>` のOGP・descriptionメタタグ（`og-image.jpg` 参照）を必ず保持すること（照準リング・方向ロック・タップ配置・キーボード操作/`role="group"`/`#sr-live`・GoatCounter・`?dev=1` と同列の保持項目）。
